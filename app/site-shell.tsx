...
/**
 * Renders one section of the public site. Each section is its own route, so
 * `page` comes in as a prop rather than local state.
 *
 * `setPage` is kept as the internal navigation API — it now pushes a real route
 * instead of flipping state, so every existing call site keeps working.
 */
export function SiteShell({ page }: { page: Page }) {
  ...
  // Old shareable hash links (/#cats) redirect to the real route.
  useEffect(() => {
    const slug = window.location.hash.replace(/^#/, "").toLowerCase();
    const target = LEGACY_HASH_TO_PATH[slug];
    if (target && target !== window.location.pathname) router.replace(target);
  }, [router]);

  // "Adopt a cat" deep-links straight into the adoptable filter. Validated, so a
  // hand-typed ?filter= can't put the page into an unknown state.
  const rawFilter = searchParams.get("filter");
  const catFilter: CatFilter =
    CAT_FILTERS.some((f) => f.value === rawFilter) ? (rawFilter as CatFilter) : "All";

  // How-to-Help sections are linkable as /how-to-help#ways-to-give.
  useEffect(() => {
    if (page !== "How to Help") return;
    const hash = window.location.hash.replace(/^#/, "");
    if (hash) setHelpTarget(hash);
  }, [page]);

  const goToAdoptable = () => router.push("/cats?filter=adoptable");
  const goToHelp = (section: string) => router.push(`/how-to-help#${section}`);
  ...
  return (
    <nav className="nav" data-screen-label="Site Nav">
      <div className="nav-inner">
        <Link href="/" className="nav-logo" aria-label="MeanKat Cafe — home">
        ...
        <div className={`nav-links ${mobileOpen ? "open" : ""}`}>
          {NAV_LINKS.flatMap((l) => {
            // Real hrefs, so search engines can crawl every section.
            const link = (
              <Link
                key={l}
                href={pathForPage(l)}
