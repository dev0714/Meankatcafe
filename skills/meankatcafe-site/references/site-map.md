...
- Hero background uses 18 randomly scattered paw icons from `public/paw-icons.png` (a 3-col × 2-row sprite sheet of 6 cartoon paw variants). Each paw is an absolutely-positioned `<div>` with `backgroundSize: "300% 200%"` and `backgroundPosition` calculated from col/row to isolate one variant. `mix-blend-mode: multiply` dissolves white backgrounds. Variant 0 (white outline paw) is skipped — it renders as bare lines under multiply. Positions are zoned: ~1/3 go to the left edge (x 0–16%), ~2/3 go to the right (x 56–100%), keeping paws off the hero text column. Seeded LCG (seed 2847) keeps layout deterministic.
