...
- Hero background uses an inline SVG with 48 randomly scattered cat paw silhouettes generated by `seededPawPrints()` (LCG seeded at 2847). The paw shape (`<defs><g id="cat-paw">`) is a cartoon-style mitt: outer rounded body with 3 toe bumps, inner pads (1 large main pad + 3 toe pads) cut through as negative space using `fillRule="evenodd"`. Centered at origin via `translate(-50,-52)`. Positions/rotations/scales/opacities are deterministic so they don't re-randomise on render. `paw-pattern.svg` in `public/` holds the canonical single-paw shape for reference.
