...
- Hero background uses an inline SVG with 22 randomly scattered cat paw silhouettes (sparse) generated by `seededPawPrints()` (LCG seeded at 2847). The paw shape (`<defs><g id="cat-paw">`) is a cartoon-style mitt: ONE continuous clockwise outer path tracing the rounded body + 3 toe bumps (no overlapping subpaths, which would create holes via evenodd). Inner pads (1 large main pad + 3 toe pads) are separate enclosed subpaths that punch holes via `fillRule="evenodd"`. Centered at origin via `translate(-50,-51)`. Deterministic positions/rotations/scales/opacities so they don't re-randomise on render. `paw-pattern.svg` matches the same path for reference.
