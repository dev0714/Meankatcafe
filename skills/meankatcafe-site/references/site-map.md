...
- Hero background uses 20 randomly scattered paw icons from `public/paw-icons.png` (a 3-col × 2-row sprite sheet of 6 cartoon paw variants). Each paw is an absolutely-positioned `<div>` with `backgroundSize: "300% 200%"` and `backgroundPosition` calculated from the column/row to isolate one variant. `mix-blend-mode: multiply` dissolves the white image background into the hero gradient. Positions, rotations, sizes, opacities, and variant index are generated by `seededPawPrints()` (LCG seed 2847) — deterministic so they never re-randomise on render.
