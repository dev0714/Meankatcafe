...
- Hero background uses an inline SVG with 48 randomly scattered cat paw silhouettes generated by `seededPawPrints()` (LCG seeded at 2847). The paw shape is defined in a `<defs><g id="cat-paw">` block — real anatomy: lobed metacarpal pad + 4 rotated ellipse toe pads. Positions/rotations/scales/opacities are deterministic so they don't re-randomize on render. `paw-pattern.svg` in `public/` now holds the canonical single-paw SVG shape for reference.
